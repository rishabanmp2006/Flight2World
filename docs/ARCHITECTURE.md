...
> **Historical note.** This layout was flat and experimental until the
> Phase 5B restructure, which implemented the target structure in section
> D.1: the experiment scripts moved from the repository root to
> `experiments/`, the dataset moved from `test/` to `data/benchmark/`, and
> this document moved to `docs/`. References to `test/` elsewhere in this
> document should be read as `data/benchmark/`.
...
├── .github/workflows/ci.yml     tests, lint, repo-hygiene gates
│
├── core/                        the library (16 modules, ~4.3k lines)
├── tests/                       unit suite (14 files, ~3.5k lines)
│
├── docs/
│   └── ARCHITECTURE.md          ← THIS DOCUMENT
│
├── experiments/                 ← frozen research lineage (DO NOT MODIFY)
│   ├── README.md                  version history + re-run instructions
│   ├── fusion_v10.py       992 lines   ← CURRENT BASELINE
│   ├── fusion_v9.py        991 lines
│   ├── fusion_55_v8.py    1187 lines
│   ├── fusion_15_v6.py    1631 lines
│   ├── fusion_15_v5.py    1433 lines
│   ├── fusion_15_v4.py    1835 lines
│   ├── fusion_15.py       1384 lines
│   ├── fusion_v3.py       1248 lines
│   ├── fusion_v2.py        701 lines
│   ├── confidence_v10.py   610 lines
│   ├── confidence_v9.py    610 lines
│   ├── compare_v9_v10.py   290 lines
│   ├── analyze_v9.py       157 lines
│   ├── clean_v7.py         313 lines
│   └── calibration_test.py 502 lines
│
├── data/
│   ├── README.md
│   └── benchmark/               ← data + results (DO NOT MODIFY)
│       ├── frames/              86 JPG frames (1280x720, ~16MB)
...
│       └── *.log                3 run logs
│
├── outputs/                     ← generated pipeline runs (git-ignored)
└── .venv/                       ← virtualenv (git-ignored)
```

**Not in git:** `outputs/` (including the 51MB-per-run COLMAP feature
databases), `__pycache__/`, `*.pyc`, `.DS_Store`, generated validation
reports, and `.venv/`. CI fails the build if any of them are staged, or if
any tracked file exceeds 20MB.
...
> **Status: implemented.** See section A.1 for the layout as it now
> stands. The `app/` and `viewer/` packages below remain future work;
> orchestration currently lives in `core/pipeline.py`.
...
class PipelineConfig:
    # Paths (repo-relative; override the dataset root with the
    # FLIGHT2WORLD_DATA environment variable)
    data_root: Path = DATA_ROOT  # default: {repo}/data/benchmark
    frame_dir: str = ""          # default: {data_root}/frames
    sparse_dir: str = ""         # default: {data_root}/sparse_txt
    output_dir: str = ""         # default: outputs/<run>
...
```bash
# 1. Run the original V10 (needs the legacy symlinks --
#    see experiments/README.md)
python experiments/fusion_v10.py
cp data/benchmark/fused_v10_diag.json /tmp/diag_original.json

# 2. Run the refactored pipeline
python -m core.pipeline            # or drive Pipeline() from Python
cp outputs/<run>/diagnostics/fusion.json /tmp/diag_refactored.json
